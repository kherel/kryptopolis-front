...
import "./Home.scss";
import HomePortfolio from "templates/Home/HomePortfolio/HomePortfolio";
...
class Home extends Component {
  render() {
    const {
      news,
      articles: { ids: aIds, entities: aEntities },
      videos: { ids: vIds, entities: vEntities }
    } = this.props;
    ...
    return (
      <div>
        <A_Container mix={cn()} padding="thin">
          ...
          <O_Articles
            mix={cn("articles")}
            articles={{ ids: aIds.slice(0, 6), entities: aEntities }}
          />
          <O_Video
            mix={cn("video")}
            {...{ ids: vIds.slice(0, 2), entities: vEntities }}
          />
        ...
        <HomePortfolio mix={cn("portfolio")} />
      </div>
    );
  }
}
