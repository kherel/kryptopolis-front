...
class MainLayout_Header extends Component {
    ...
    if (loggedIn) {
      return (
        <div className={cn("login", { loggedIn })}>
          <A_Link href={"/"} mix={cn("login-profile")}>
            {email}
          </A_Link>
          <A_Btn mix={cn("login-logout-btn")}>
            <A_Svg
              name={"logout"}
              className={cn("login-logout-icon")}
  ...
  getMobileMenu() {
    ...
    return (
      <div className={cn("mobile-menu", { open: true })}>
        {styles}
        <A_Svg
          name={"close"}
  ...
  render() {
    const { mix, tickers:{up, down, main, entities} } = this.props;
    ...
    return (
      <A_Container mix={cn([mix])}>
        ...
        <div className={cn("slogan")}>
          AKTUALNE WIADOMOŚCI <br />O ŚWIECIE KRYPTOWALUTY
        </div>
        <MainLayoutTicker {...{ entities, up, down, mix: cn("ticker") }} />
        <MainLayoutCurrency {...{ entities, main, mix: cn("currencies") }} />
        <A_Svg
          name={"burger"}
          className={cn("btn")}
...
MainLayout_Header.propTypes = {
  ...
  tickers: T.shape({
    ...
    up: T.array.isRequired,
    down: T.array.isRequired,
    main: T.array.isRequired,
    entities: T.object.isRequired
  }).isRequired
};
